...
import { 
  ...
  Clock, Play, Square, Plus, Receipt, List, Filter, FileCheck, Mail, MoreHorizontal, Download, Phone, Briefcase as BriefcaseIcon, BadgeCheck, BookOpen, ClipboardList, Loader, Contact
} from 'lucide-react';
...
// --- CONFIGURATION ---
// Note: In a local environment, replace this null check with your actual config object
...
const SEED_TRUSTS = [
  { id: 't1', name: "The Smith Family Dynasty Trust", situs: "Nevada", type: "Irrevocable", status: "active", assets: 14200000, progress: 35, nextTask: "Review 706 Draft", ein: "88-1234567", dateOfDeath: "Oct 12, 2024" },
  { id: 't2', name: "Estate of Arthur P. Jenkins", situs: "California", type: "Revocable", status: "critical", assets: 2100000, progress: 15, nextTask: "Marshalling", ein: "99-7654321", dateOfDeath: "Nov 01, 2024" },
  { id: 't3', name: "Llewellyn Charitable Trust", situs: "Wyoming", type: "CRT", status: "active", assets: 5600000, progress: 45, nextTask: "Distribution", ein: "45-9988776", dateOfDeath: "N/A" }
];

const SEED_TASKS = [
    { title: "File 706 Return", trustId: 't1', priority: "High", status: "todo", due: "2 Days" },
    { title: "Approve Appraiser Invoice", trustId: 't2', priority: "Medium", status: "todo", due: "Today" },
    { title: "Draft Response to Greg", trustId: 't1', priority: "High", status: "in-progress", due: "ASAP" },
    { title: "Quarterly Distribution", trustId: 't3', priority: "Low", status: "todo", due: "Next Week" },
    { title: "Review Investment Performance", trustId: 't1', priority: "Medium", status: "done", due: "Yesterday" },
];

const SEED_VENDORS = [
    { name: "Sarah Jenkins", firm: "Jenkins Law Group, LLP", role: "Attorney", rate: 450, phone: "(555) 123-4567", email: "sarah@jenkinslaw.com", engagedOn: ['t1', 't2'], status: "Active" },
    { name: "Robert 'Bob' Vance", firm: "Vance & Associates CPAs", role: "Accountant (CPA)", rate: 325, phone: "(555) 987-6543", email: "bob@vancecpa.com", engagedOn: ['t1', 't3'], status: "Active" },
    { name: "Cushman & Wakefield", firm: "Cushman Commercial", role: "Appraiser", rate: 5000, type: "Flat Fee", phone: "(555) 555-5555", email: "appraisals@cushman.com", engagedOn: ['t2'], status: "Pending" },
    { name: "Northern Trust", firm: "Northern Trust Corp", role: "Investment Advisor", rate: 100, type: "BPS", phone: "(800) 123-0000", email: "wealth@ntrs.com", engagedOn: ['t1', 't3'], status: "Active" },
];

const ASSETS_MOCK = [
  { id: 'a1', trustId: 't1', name: "123 Highland Ave", type: "Real Estate", currentValue: 2600000, notes: "Appraisal low." },
  { id: 'a2', trustId: 't1', name: "Chase Checking", type: "Cash", currentValue: 425000, notes: "Operating." },
  { id: 'a3', trustId: 't1', name: "Smith Family LLC", type: "Business", currentValue: 4500000, notes: "Discounted." },
  { id: 'a4', trustId: 't1', name: "NVIDIA Corp", type: "Securities", currentValue: 6800000, notes: "Verified." },
  { id: 'a5', trustId: 't1', name: "Coinbase Wallet", type: "Digital Asset", currentValue: 920000, notes: "Secured. Custodian: Coinbase." }
];
...
const TRANSACTIONS = [
    ...
    { id: 5, date: "2024-11-25", type: "Receipt", category: "Gain", desc: "Sale of Ford Truck", incomeRx: 15000.00, incomeDx: 0, princRx: 0, princDx: 0 },
];
...
const BILLING_ENTRIES = [
    ...
    { id: 4, date: "Nov 28", desc: "Meeting with CPA", hours: 1.0, rate: 250, total: 250.00 },
    { id: 5, date: "Nov 25", desc: "Correspondence - Sarah", hours: 0.2, rate: 250, total: 50.00 },
];
...
const VENDORS_MOCK = [
    { id: 1, name: "Sarah Jenkins", firm: "Jenkins Law Group, LLP", role: "Attorney", rate: 450, phone: "(555) 123-4567", email: "sarah@jenkinslaw.com", engagedOn: [1, 2], status: "Active" },
    { id: 2, name: "Robert 'Bob' Vance", firm: "Vance & Associates CPAs", role: "Accountant (CPA)", rate: 325, phone: "(555) 987-6543", email: "bob@vancecpa.com", engagedOn: [1, 3], status: "Active" },
    { id: 3, name: "Cushman & Wakefield", firm: "Cushman Commercial", role: "Appraiser", rate: 5000, type: "Flat Fee", phone: "(555) 555-5555", email: "appraisals@cushman.com", engagedOn: [2], status: "Pending" },
    { id: 4, name: "Northern Trust", firm: "Northern Trust Corp", role: "Investment Advisor", rate: 100, type: "BPS", phone: "(800) 123-0000", email: "wealth@ntrs.com", engagedOn: [1, 3], status: "Active" },
];
...
const INCOMING_EMAILS = [
  ...
  { id: 'e3', sender: "broker@schwab.com", subject: "Llewellyn Retitling", date: "Yesterday", aiConfidence: 0.95, body: "Account x8829 has been retitled.", suggestedTrust: 't3', suggestedTag: "Admin" }
];

const getAccountingData = (trustId) => {
    let charges = [
        { id: 'A', name: "Property on Hand (Start)", value: 14200000 },
        { id: 'B', name: "Receipts (Income)", value: 150000 },
        { id: 'C', name: "Gains on Sales", value: 50000 },
        { id: 'D', name: "Net Income from Trade/Business", value: 0 },
    ];
    let credits = [
        { id: 'E', name: "Disbursements (Expenses)", value: 25000 },
        { id: 'F', name: "Losses on Sales", value: 0 },
        { id: 'G', name: "Distributions to Beneficiaries", value: 0 },
        { id: 'H', name: "Property on Hand (Ending)", value: 14375000 },
    ];
    if (trustId === 't2') {
        credits[0].value = 25500; 
    }
...
const PrivilegeToggle = ({ isPrivileged, setIsPrivileged, className = "" }) => {
  return (
    <div className={`flex items-center space-x-3 rounded-full px-4 py-2 shadow-sm border transition-all ${className} ${isPrivileged ? 'bg-red-50 border-red-200' : 'bg-racing-green-light border-racing-green'}`}>
        <span className={`text-xs font-bold font-serif ${isPrivileged ? 'text-red-800' : 'text-racing-green'}`}>
            {isPrivileged ? 'FIDUCIARY (Privileged)' : 'BENEFICIARY (Safe)'}
        </span>
...
const TaskCommandCenter = ({ tasks, trusts, onMoveTask }) => {
    ...
    const TaskCard = ({ task }) => (
        <div className="kanban-card group flex flex-col justify-between h-full bg-white p-4 border border-stone-200 rounded shadow-sm hover:shadow-md transition-all">
            <div>
                ...
                <p className="text-xs text-stone-500 font-serif italic mb-3">{getTrustName(task.trustId)}</p>
            </div>
            <div className="flex justify-end pt-2 border-t border-stone-100 opacity-20 group-hover:opacity-100 transition-opacity">
                {task.status !== 'done' && (
                    <button onClick={() => onMoveTask(task.id, task.status === 'todo' ? 'in-progress' : 'done')} className="text-xs text-racing-green font-bold flex items-center hover:underline">
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full">
                <div className="bg-stone-50 rounded-lg p-4 border border-stone-200 min-h-[500px]"><h3 className="font-bold text-stone-700 uppercase tracking-widest text-xs mb-4 flex items-center">To Do ({tasks.filter(t => t.status === 'todo').length})</h3><div className="space-y-2">{tasks.filter(t => t.status === 'todo').map((task, i) => <TaskCard key={i} task={task} />)}</div></div>
                <div className="bg-stone-50 rounded-lg p-4 border border-stone-200 min-h-[500px]"><h3 className="font-bold text-blue-700 uppercase tracking-widest text-xs mb-4 flex items-center">In Progress ({tasks.filter(t => t.status === 'in-progress').length})</h3><div className="space-y-2">{tasks.filter(t => t.status === 'in-progress').map((task, i) => <TaskCard key={i} task={task} />)}</div></div>
                <div className="bg-stone-50 rounded-lg p-4 border border-stone-200 min-h-[500px]"><h3 className="font-bold text-green-700 uppercase tracking-widest text-xs mb-4 flex items-center">Completed ({tasks.filter(t => t.status === 'done').length})</h3><div className="space-y-2">{tasks.filter(t => t.status === 'done').map((task, i) => <TaskCard key={i} task={task} />)}</div></div>
            </div>
        </div>
    );
};

const BillingModule = ({ invoices = [] }) => {
...
const CourtAccounting = ({ trust, isPrivileged }) => {
...
const AdminConsole = () => {
  ...
  const modules = [{ id: 'core', label: 'Core Ledger & Vault', tiers: ['standard', 'pro', 'dynasty'], icon: FileText }, { id: 'privilege', label: 'Privilege Firewall', tiers: ['standard', 'pro', 'dynasty'], icon: Shield }, { id: 'oracle', label: 'AI Fiduciary Mind', tiers: ['pro', 'dynasty'], icon: BrainCircuit }, { id: 'legacy', label: 'Digital Successor Protocol', tiers: ['dynasty'], icon: Hourglass }, { id: 'audit', label: 'Court-Ready Audit Logs', tiers: ['pro', 'dynasty'], icon: History }, { id: 'nexus', label: 'Multi-State Nexus Watchdog', tiers: ['dynasty'], icon: Globe }, { id: 'accounting', label: 'Court Accounting Engine', tiers: ['pro', 'dynasty'], icon: Scale }];
  return (
    <div className="space-y-8 animate-fadeIn">
       ...
       <div className="walnut-card"><h3 className="font-serif font-bold text-lg text-stone-800 mb-6 border-b border-stone-100 pb-2">Module Configuration</h3><div className="space-y-4">{modules.map((mod) => { const isEnabled = mod.tiers.includes(selectedTier); return (<div key={mod.id} className={`flex items-center justify-between p-4 rounded border ${isEnabled ? 'bg-white border-stone-200' : 'bg-stone-50 border-stone-100 opacity-50'}`}><div className="flex items-center space-x-4"><div className={`p-2 rounded ${isEnabled ? 'bg-racing-green-light text-racing-green' : 'bg-stone-200 text-stone-400'}`}><mod.icon size={20} /></div><div><p className={`font-bold text-sm ${isEnabled ? 'text-stone-800' : 'text-stone-400'}`}>{mod.label}</p></div></div><div><label className="toggle-switch"><input type="checkbox" className="toggle-checkbox" checked={isEnabled} disabled={!isEnabled} readOnly /><span className="toggle-slider"></span></label></div></div>) })}</div></div>
    </div>
  );
};

const VendorRolodex = ({ vendors }) => {
  const getTrustNames = (ids) => ids.map(id => TRUSTS.find(t => t.id === id)?.name).filter(Boolean);
  return (<div className="space-y-6 animate-fadeIn"><div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-stone-200 pb-6"><div><h1 className="font-serif text-3xl font-bold text-stone-900">Professional Directory</h1><p className="text-stone-500 font-serif italic">Your Ecosystem of Experts</p></div><div className="flex space-x-2"><button className="flex items-center px-4 py-2 bg-white border border-stone-300 rounded text-stone-600 font-bold text-sm hover:bg-stone-50 transition"><Filter size={16} className="mr-2"/> Filter</button><button className="flex items-center px-4 py-2 bg-racing-green text-white rounded font-bold text-sm hover:bg-stone-800 transition shadow-md"><Plus size={16} className="mr-2"/> Add Professional</button></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6">{vendors.map((vendor, i) => (<div key={i} className="walnut-card p-6 hover:shadow-md transition-shadow group"><div className="flex justify-between items-start mb-4"><div className="flex items-center space-x-4"><div className="w-12 h-12 rounded-full bg-stone-100 flex items-center justify-center font-bold font-serif text-stone-600 text-lg border border-stone-200">{vendor.name.charAt(0)}</div><div><h3 className="font-bold text-stone-900 font-serif text-lg">{vendor.name}</h3><div className="flex items-center text-xs text-stone-500 font-serif italic"><BriefcaseIcon size={12} className="mr-1"/> {vendor.firm}</div></div></div><span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider border ${vendor.role.includes('Attorney') ? 'bg-red-50 text-red-800 border-red-100' : 'bg-stone-100 text-stone-600 border-stone-200'}`}>{vendor.role}</span></div><div className="space-y-2 mb-6"><div className="flex items-center text-sm text-stone-600"><Mail size={14} className="mr-2 text-racing-green"/> {vendor.email}</div><div className="flex items-center text-sm text-stone-600"><Phone size={14} className="mr-2 text-racing-green"/> {vendor.phone}</div></div><div className="pt-4 border-t border-stone-100"><p className="text-[10px] text-stone-400 uppercase font-bold mb-2">Active Engagements</p><div className="flex flex-wrap gap-2">{getTrustNames(vendor.engagedOn).map((trustName, idx) => (<span key={idx} className="inline-flex items-center px-2 py-1 rounded bg-racing-green-light text-racing-green text-xs font-bold border border-racing-green/20"><BadgeCheck size={10} className="mr-1"/> {trustName}</span>))}</div></div></div>))}</div></div>);
};

const Sidebar = ({ activeTab, setActiveTab, isPrivileged, setIsPrivileged, isMobile, isOpen, setIsOpen }) => {
...
const MorningBriefing = ({ onAnalyzeRisk, userName = "Larry" }) => {
  ...
  return (
    <div className="walnut-card mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-4 bg-yellow-50 border-l-4 border-yellow-500 rounded-r-md"><div className="flex justify-between items-start"><h3 className="font-bold text-yellow-800 font-serif text-sm">Tax Cliff</h3><AlertOctagon size={16} className="text-yellow-600" /></div><p className="text-yellow-700 text-xs mt-1 font-serif">Exemption drops in ~390 Days.</p><button onClick={() => onAnalyzeRisk('tax')} className="mt-2 text-xs font-bold text-yellow-800 underline cursor-pointer">Analyze Strategy</button></div>
...
// Vault: Read-Only, Finalized Docs, No Toggle
const Vault = () => {
    const [viewDoc, setViewDoc] = useState(null);
...
// ... (Dashboard, DigitalSuccessor, App maintained from v1.4.0) ...
const DigitalSuccessor = () => (
  <div className="space-y-6 animate-fadeIn">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {TRUSTS.map((trust) => (
          <div key={trust.id} onClick={() => onSelectTrust(trust.id)} className="walnut-card cursor-pointer hover:shadow-md">
            ...
            <main className="p-4 md:p-8 w-full">
               <div className="max-w-6xl mx-auto">
                 {selectedTrustId ? (
                   ...
                   <>
                     {activeTab === 'dashboard' && <Dashboard isPrivileged={isPrivileged} onSelectTrust={setSelectedTrustId} onAnalyzeRisk={handleAnalyzeRisk} />}
